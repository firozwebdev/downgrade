...
<div class="page-title-overlap pt-4" style="background-image: url('{{ url('/') }}/public/storage/settings/{{ $allsettings->site_banner }}');">
      <div class="container d-lg-flex justify-content-between py-2 py-lg-3">
        ...
        <div class="order-lg-1 pr-lg-4 text-center text-lg-left">
          ...
          <p class="product_short_desc">{{ $item['view']->product_short_desc }}</p>
          @endif
        </div>
      </div>
    </div>
            ...
            <div class="pt-2 px-4 pr-lg-0 pl-xl-5">
              ...
              <div class="d-flex flex-wrap justify-content-between align-items-center border-top pt-3">
                <div class="py-2 mr-2">
                  ...
                  <a class="btn btn-outline-accent btn-sm" href="{{ url('/preview') }}/{{ $item['view']->product_slug }}" target="_blank"><i class="dwg-eye font-size-sm mr-2"></i>{{ Helper::translation(4143,$translate,'') }}</a>
                  @endif
                  @if($item['view']->product_video_url != '') 
                  <a class="btn btn-outline-accent btn-sm popupvideo" href="{{ $item['view']->product_video_url }}"><i class="dwg-video font-size-lg mr-2"></i>{{ Helper::translation(4146,$translate,'') }}</a>
                  @endif
                  @if(Auth::guest())
                  ...
                  <a class="btn btn-outline-accent btn-sm" href="{{ url('/item') }}/{{ base64_encode($item['view']->product_id) }}/favorite/{{ base64_encode($item['view']->product_liked) }}"><i class="dwg-heart font-size-lg mr-2"></i>{{ Helper::translation(4149,$translate,'') }}</a>
                  @endif
                  @endif
                  </div>
                ...
                <i class="dwg-edit font-size-lg text-muted"></i> <a href="{{ $allsettings->product_reporting_url }}" target="_blank">{{ Helper::translation('63f75734b5acc',$translate,'Report this item') }}</a>
                </div>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header border-bottom-0">
        <h5 class="modal-title" id="exampleModalLabel">{{ Helper::translation('63f75734b5acc',$translate,'Report this item') }}</h5>
       ...
       {{ csrf_field() }}
        <div class="modal-body">
          ...
          <div class="form-group">
            <label for="email1">{{ Helper::translation(4002,$translate,'') }} <span class="text-danger">*</span></label>
            <input type="text" class="form-control" name="report_fullname" data-bvalidator="required">
            
          </div>
          <div class="form-group">
            <label for="email1">{{ Helper::translation(4023,$translate,'') }} <span class="text-danger">*</span></label>
          ...
          @endif
          <div class="form-group">
            <label for="password1">{{ Helper::translation('63f757e209c67',$translate,'Issue Type') }} <span class="text-danger">*</span></label>
            <select class="form-control" name="report_issue_type" data-bvalidator="required">
            <option value="Copyright Issue">{{ Helper::translation('63f757b3ddf7b',$translate,'Copyright Issue') }}</option>
            <option value="Payment Issue">{{ Helper::translation('63f7579d7d2b7',$translate,'Payment Issue') }}</option>
            <option value="Item or File Problem">{{ Helper::translation('63f757a4e3b09',$translate,'Item or File Problem') }}</option>
            <option value="General Issue">{{ Helper::translation('63f757ac5ad66',$translate,'General Issue') }}</option>
            <option value="Suggestion">{{ Helper::translation('63f757bd179a7',$translate,'Suggestion') }}</option>
            </select>
          </div>
          <div class="form-group">
            <label for="email1">{{ Helper::translation(4227,$translate,'') }} <span class="text-danger">*</span></label>
            <input type="text" class="form-control" name="report_subject" data-bvalidator="required">
            
          </div>
          <div class="form-group">
            <label for="email1">{{ Helper::translation(4005,$translate,'') }} <span class="text-danger">*</span></label>
        ...
        <div class="modal-footer border-top-0 d-flex justify-content-center">
          <button type="submit" class="btn btn-success">{{ Helper::translation(4572,$translate,'') }}</button>
        </div>
      </form>
    </div>
  </div>
               ...
               @if(Auth::guest())
                    ...
                    <form action="{{ route('support') }}" class="support_form media-body needs-validation" id="support_form" method="post" enctype="multipart/form-data">
                                            ...
                                            <div class="form-group">
                                                    ...
                                                    <input type="text" id="support_subject" name="support_subject" class="form-control" placeholder="{{ Helper::translation(4230,$translate,'') }}" data-bvalidator="required">                     </div>
                                                <div class="form-group">
                                                    <label for="supmsg">{{ Helper::translation(4005,$translate,'') }} </label>
                                                    <textarea class="form-control" id="support_msg" name="support_msg" rows="5" placeholder="{{ Helper::translation(4233,$translate,'') }}" data-bvalidator="required"></textarea>                          </div>
     ...
     @endif
          ...
          <aside class="col-lg-4">
            ...
            <div class="cz-sidebar-static h-100 ml-auto border-left">
               ...
               <div align="center">
                   @if (Auth::check())
                   <a href="{{ URL::to('/item') }}/download/{{ base64_encode($item['view']->product_token) }}" class="btn btn-primary btn-sm" download> <i class="fa fa-download"></i> {{ Helper::translation(4161,$translate,'') }} ({{ $dcount }})</a>
                   @endif
                   @if(Auth::guest())
               ...
               <div class="bg-secondary rounded p-3 mb-4">
                   ...
                   <p>{{ Helper::translation('63918549dc4c7',$translate,'This product is one of the Subscribe Users Free Download Files. You are able to download this product for free for a Unlimited time. Updates and support are only available if you purchase this item.') }}</p>
                   <div align="center">
                   <a href="{{ URL::to('/item') }}/download/{{ base64_encode($item['view']->product_token) }}" class="btn btn-primary btn-sm"> <i class="fa fa-download"></i> {{ Helper::translation(4161,$translate,'') }} ({{ $dcount }})</a>
                   </div>
                   @else
                   <p>{{ Helper::translation('6391878d23c34',$translate,'Subscribe to unlock this product, plus millions of creative assets with unlimited downloads.') }}</p>
                   <div align="center">
                   <a href="{{ URL::to('/subscription') }}" class="btn btn-primary btn-sm"> <i class="fa fa-download"></i> {{ Helper::translation('6395ce234e0b0',$translate,'Renew Subscription') }} ({{ $dcount }})</a>
                   </div>
                   @endif
                   @else
                   <p>{{ Helper::translation('6391878d23c34',$translate,'Subscribe to unlock this product, plus millions of creative assets with unlimited downloads.') }}</p>
                   <div align="center">
                   <a href="{{ URL::to('/subscription') }}" class="btn btn-primary btn-sm"> <i class="fa fa-download"></i> {{ Helper::translation('639187c84247c',$translate,'Subscribe to download') }} ({{ $dcount }})</a>
                   </div>
                   @endif
                   @endif
                   @if(Auth::guest())
              ...
              <p class="mt-2 mb-3"><a href="javascript:void(0)" data-toggle="modal" data-target="#myModal" class="font-size-xs">{{ $page['view']->page_title }}</a></p>
                ...
                @if(Auth::guest())
                <button type="submit" class="btn btn-primary btn-shadow btn-block mt-4"><i class="dwg-cart font-size-lg mr-2"></i>{{ Helper::translation(4182,$translate,'') }}</button>
                @endif
                @if (Auth::check())
                @if($item['view']->user_id == Auth::user()->id)
                <a href="{{ URL::to('/admin/edit-product') }}/{{ $item['view']->product_token }}" class="btn btn-primary btn-shadow btn-block mt-4"><i class="dwg-cart font-size-lg mr-2"></i>{{ Helper::translation(4185,$translate,'') }}</a>
                @else
              ...
              <div class="bg-secondary rounded p-3 mb-2">
                ...
                <div class="font-size-ms text-muted">{{ $item['view']->product_fake_stars }} {{ Helper::translation(4188,$translate,'') }}</div>
                @else
                <div class="font-size-ms text-muted">{{ $getreview }} {{ Helper::translation(4188,$translate,'') }}</div>
                @endif
              </div>
              <div class="bg-secondary rounded p-3 mb-4"><i class="dwg-chat h5 text-muted align-middle mb-0 mt-n1 mr-2"></i><span class="d-inline-block h6 mb-0 mr-1">{{ $comment_count }}</span><span class="font-size-sm">{{ Helper::translation(4191,$translate,'') }}</span></div>
              <ul class="list-unstyled font-size-sm">
                <li class="d-flex justify-content-between mb-3 pb-3 border-bottom"><span class="text-dark font-weight-medium">{{ Helper::translation(4194,$translate,'') }}</span><span class="text-muted">{{ date('d M Y', strtotime($item['view']->product_update)) }}</span></li>
                <li class="d-flex justify-content-between mb-3 pb-3 border-bottom"><span class="text-dark font-weight-medium">{{ Helper::translation(4197,$translate,'') }}</span><span class="text-muted">{{ date('d M Y', strtotime($item['view']->product_date)) }}</span></li>
                <li class="d-flex justify-content-between mb-3 pb-3 border-bottom"><span class="text-dark font-weight-medium">{{ Helper::translation(4200,$translate,'') }}</span><a class="product-meta" href="{{ URL::to('/shop/category') }}/{{ $item['view']->category_slug }}">{{ $item['view']->category_name }}</a></li>
                ...
                <?php /*?><li class="d-flex justify-content-between mb-3 pb-3 border-bottom"><span class="text-dark font-weight-medium title-width">{{ Helper::translation(4206,$translate,'') }}</span><span class="text-muted text-right">       @php $browse_info = ""; @endphp

      ...
      <div class="bg-light box-shadow-lg rounded-lg overflow-hidden">
        <div class="row">
          ...
          <form action="{{ route('checkout') }}" class="needs-validation" id="checkout_form" method="post" enctype="multipart/form-data">
            ...
            <div class="pt-2 px-4 pr-lg-0 pl-xl-5">
              ...
              <div class="widget mb-3 d-lg-none">
                ...
                <ul class="list-unstyled font-size-sm py-3">
                  ...
                  @endphp
                  <li class="d-flex justify-content-between align-items-center font-size-base"><span class="mr-2">{{ Helper::translation('6401e40d3aa6a',$translate,'Discount Price') }}</span><span class="text-right"> - {{ $allsettings->site_currency_symbol }} {{ $coupon_discount }}</span></li>
                  @else
                  @php 
                  ...
                  @endphp
                  @endif
                  <li class="d-flex justify-content-between align-items-center font-size-base"><span class="mr-2">{{ Helper::translation(3963,$translate,'') }}</span><span class="text-right">{{ $allsettings->site_currency_symbol }} {{ $final }}</span></li>
                </ul>
              </div>
              <div class="accordion mb-2" id="payment-method" role="tablist">
                ...
                <div class="card">
                  ...
                  @if($payment == 'stripe')
                    ...
                    @if($payment == 'wallet')
                    <div class="card-body font-size-sm custom-control custom-radio">
                      <p><span class='font-weight-medium'><input id="opt1-{{ $payment }}" name="payment_method" type="radio" class="custom_radio" value="{{ $payment }}" data-bvalidator="required"> {{ Helper::translation('6405c972ddf8b',$translate,'Wallet') }}</span> - ({{ $allsettings->site_currency_symbol }}{{ Auth::user()->earnings }})</p>
                      <button class="btn btn-primary" type="submit">{{ Helper::translation('6405c96989553',$translate,'Checkout with Wallet') }}</button>
                    </div>
                    @endif
                    @endif
                    @if($payment == 'paystack')
                    ...
                    <div class="card-body font-size-sm custom-control custom-radio">
                      ...
                      <button class="btn btn-primary" type="submit">{{ Helper::translation('6437ad79c306c',$translate,'Checkout with Local Bank') }}</button>
                    </div>
                    @endif
                    @if($payment == 'razorpay')
                    <div class="card-body font-size-sm custom-control custom-radio">
                    ...
                    <div class="card-body font-size-sm custom-control custom-radio">
                      ...
                      <button class="btn btn-primary" type="submit">{{ Helper::translation('64bbc871280de',$translate,'Checkout with Mercadopago') }}</button>
                    </div>
                    @endif
                  </div>
                </div>
            ...
            <input type="hidden" name="amount" value="{{ base64_encode($final) }}">
            ...
            <div class="cz-sidebar-static h-100 ml-auto border-left">
              <div class="widget mb-3">
                ...
                <ul class="list-unstyled font-size-sm pt-3 pb-2 border-bottom">
                  ...
                  $priceamount = $new_price;
                  @endphp
                  <li class="d-flex justify-content-between align-items-center"><span class="mr-2">{{ Helper::translation('6401e40d3aa6a',$translate,'Discount Price') }}</span><span class="text-right"> - {{ $allsettings->site_currency_symbol }} {{ $coupon_discount }}</span></li>
                  @else
                  @php 
                  $final = $subtotal+$allsettings->site_extra_fee; 
                  $last_price =  $subtotal;
